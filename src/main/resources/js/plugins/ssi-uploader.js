...
(function(root, factory) {
    ...
    Ssi_upload.prototype.init = function(element) {
        ...
        var $chooseBtn = $('' +
            ...
            '<button class="ssi-button success" id="chooseButton">' + this.language.chooseFiles + '</button>' +
            '</span>').append(element);
        var $uploadBtn = $('<button id="ssi-uploadBtn" class="ssi-button success" >' +
            '<span class="ssi-btnIn">' + this.language.upload + '&nbsp;</span>' +
            '<div id="ssi-up_loading" class="ssi-btnIn"></div></button>');
        var $clearBtn = $('<button id="ssi-clearBtn" class="ssi-button info" >' + this.language.clear +
            '</button>');
    ...
    var locale = {
        en: {
            ...
            clear: '全部删除',
            drag: '拖拽文件到此',
            sizeError: '$1 大小限制为 $2', // $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1后缀文件不支持上传', //$1=file extension ie(exe files are not supported)
